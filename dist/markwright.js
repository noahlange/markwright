...
import { defaultRules, blockRegex, inlineRegex, parserFor, ruleOutput, reactFor } from 'simple-markdown';
...
function makePage(page, columns, footnotes, header = [{ type: 'text', content: [] }]) {
    return {
        content: [
            {
                content: header,
    ...
    return {
        ...
        codeBlock: {
            ...
            react(node, _, state) {
                ...
                return (React.createElement("div", { key: state.key },
                    React.createElement(Async, { promise: content, pending: () => (React.createElement("pre", null,
                            React.createElement("code", null, node.content))), then: (v) => {
                            return (React.createElement("pre", { className: `lang lang-${node.lang}` },
                                React.createElement("code", { dangerouslySetInnerHTML: { __html: v } })));
                        } })));
            }
        },
        heading: {
        ...
        mw: {
            react(node, output) {
                return (React.createElement("div", { className: "mw", key: `mw-${node.id}` }, output(node.content)));
            }
        },
        'mw-block': {
            ...
            react(node, output) {
                return (React.createElement("div", { key: node.id, className: `block ${node.block}` }, output(node.content)));
            }
        },
        ...
        'mw-break': {
            ...
            react(node) {
                return React.createElement("div", { key: `mw-break-${node.id}`, className: "break" });
            }
        },
        'mw-column': divOf('mw-column'),
        'mw-column-separator': {
            react(node) {
                return React.createElement("hr", { key: node.id, className: "column-separator" });
            }
        },
        ...
        'mw-footnote': {
            ...
            react(node, output) {
                return node.inline ? (React.createElement("sup", { key: `${node.id}-inline` }, node.id)) : (React.createElement("span", { key: node.id, className: "footnote" },
                    node.id,
                    ". ",
                    output(node.content)));
            }
        ...
        'mw-footnotes': {
            react(node, output) {
                const str = `footnotes ${node.content && node.content.length ? '' : 'empty'}`;
                return (React.createElement("div", { key: `mw-footnote-${node.id}`, className: str }, output(node.content)));
            }
        },
        ...
        'mw-page': {
            react(node, output) {
                ...
                return (React.createElement("div", { key: `mw-page-${node.id}`, className: `page page-${node.id} ${even}` },
                    React.createElement(React.Fragment, null,
                        output(node.content),
                        React.createElement("div", { key: `mw-page-${node.id}-pagination`, className: "pagination" }, node.id))));
            }
        },
        'mw-section': {
            react(node, output) {
                return (React.createElement("div", { key: `mw-section-${node.id}`, className: `section section-${node.id}` }, output(node.content)));
            }
        },
        paragraph: {
            ...defaultRules.paragraph,
            react(node, output, state) {
                return React.createElement("p", { key: `p-${state.key}` }, output(node.content));
            }
        }
    };
};
...
function pageRenderer(sections, r) {
    ...
    return ({ index, style }) => {
        ...
        if (section) {
            ...
            return (React.createElement("div", { style: { ...style, top: top + (index + 1) * 32 } }, render(page)));
        }
...
class Markwright extends React.Component {
    ...
    static react(content, page, regions, columns, highlight) {
        ...
        const output = ruleOutput(r, 'react');
        const render = reactFor(output);
        return (React.createElement("div", { className: "section", style: { width: page ? page.width : undefined } }, render(sections)));
    }
    static reactVirtualized(content, page, regions, columns, highlight) {
        const r = Markwright.rules || (Markwright.rules = rules({ highlight }));
        const parser = parserFor(r);
        const tree = parser(content);
        const sections = transformAST(tree, regions, columns);
        ...
        const height = Markwright.ref
            ...
            : window.innerHeight;
        return (React.createElement("div", { className: "section", style: { width: page.width } },
            React.createElement(FixedSizeList, { width: page.width, height: height, itemSize: page.height, itemCount: pages }, pageRenderer(sections, r))));
    }
    ...
    static flow(manual) {
        ...
        for (const page of pages) {
            ...
            if (column) {
                const height = column.getBoundingClientRect().height - FOOTNOTE_BLOCK_HEIGHT;
    ...
    render() {
        const render = this.props.virtualized
            ...
            : Markwright.react;
        return (React.createElement("div", { className: this.state.flowed ? '' : 'unflowed', ref: e => (Markwright.ref = e) },
            React.createElement("style", { type: "text/css" }, this.state.style),
            render.call(Markwright, this.state.content || ' ', this.props.page, this.state.regions, this.props.columns, this.props.highlight)));
    }
}
...
class index extends React.Component {
    ...
    render() {
        return (React.createElement(Markwright, { value: this.props.value, page: this.config('page', { width: 96 * 8.5, height: 96 * 11 }), manual: this.config('manual', true), columns: this.config('manual', true) ? 1 : this.config('columns', 2) || 1, highlight: this.config('highlight'), flowed: this.state.flowed, regions: this.state.regions, onFlow: r => this.setState({ flowed: true, regions: r }) }));
    }
}
